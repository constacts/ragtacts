(ns ragtacts.legacy.core
  (:refer-clojure :exclude [sync chunk])
  (:require [clojure.java.io :as io]
            [clojure.string :as str]
            [ragtacts.legacy.app :as app :refer [make-app]]
            [ragtacts.legacy.collection :as collection :refer [make-collection]]
            [ragtacts.legacy.connector.folder :refer [make-folder-connector]]
            [ragtacts.legacy.connector.web-page :refer [make-web-page-connector]]
            [ragtacts.legacy.embedder.all-mini-lm-l6-v2 :refer [make-all-mini-lm-l6-v2-embedder]]
            [ragtacts.legacy.llm.llama-cpp :refer [make-llama-cpp-llm]]
            [ragtacts.legacy.memory.window :refer [make-window-chat-memory]]
            [ragtacts.legacy.prompt-template.default :refer [make-default-prompt-template]]
...
(comment
  ...
  (require '[ragtacts.legacy.core :refer [app sync chat]])
